...
class Navigation(py_trees.behaviour.Behaviour):
    def __init__(self, name, blackboard, turning=False):
    ...
    def update(self):
        ...
        if len(self.WP) == 4: # if the length of the waypoints is less than 10 then we are mapping the environment
            if rho<0.5 and abs(alphaE) < 0.02: # udpate the marker
        ...
        p1 = 2.00 # angular velocity

        if self.turning: # if we want to only turn and not move forward then set the forward velocity to 0
            p2 = 0.0 # foward velocity

            if abs(alphaE) < 0.02: # if we are in the correct orientation, even if we are not close enough, we are done
                print('FINISHED NAVIGATING')
                return py_trees.common.Status.SUCCESS
